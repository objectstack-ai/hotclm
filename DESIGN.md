...
**各段时长与按类型的「超 SLA」不在 V1.0 交付面内**（维护者裁定，2026-09-09）。语义层不收 SQL 也不收表达式（ADR-0021），没有地方做时间戳减法；且 `Field.datetime` 在 SQLite 上存 ISO 文本，`AVG()` 打在其上会静默返回平均年份而不是平均日期。两项都等平台的日期能力，跟踪于 `objectstack-ai/objectstack#16737`。在那之前 §09 交付的是：周转以各阶段到达数与审批吞吐如实呈现，SLA 以固定 30 天阈值呈现且阈值写进砖的标题（九种类型的真实 SLA 是 2/3/5/10 天，30 天高于全部，标题自陈阈值就不会被误读成按类型的违约）。⛔ 不得用应用侧日任务盖戳字段把这两项补出来 —— 那是在应用里复刻平台规则。
